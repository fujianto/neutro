...
<div class="loop-meta">
	...
	<?php } elseif ( is_search() ) { ?>
		...
		<div class="loop-description">
			<?php echo wpautop( sprintf( __( 'You are browsing the search results for "%s"', 'neutro' ), esc_attr( get_search_query() ) ) ); ?>
		</div><!-- .loop-description -->
	...
	<?php } elseif ( is_day() || is_month() || is_year() ) { ?>
		...
		<?php
			if ( is_day() )
				$date = get_the_time( __( 'F d, Y', 'neutro' ) );
			elseif ( is_month() )
				$date = get_the_time( __( 'F Y', 'neutro' ) );
			elseif ( is_year() )
				$date = get_the_time( __( 'Y', 'neutro' ) );
		?>

		<h1 class="loop-title"><?php echo $date; ?></h1>

		<div class="loop-description">
			<?php echo wpautop( sprintf( __( 'You are browsing the site archives for %s.', 'neutro' ), $date ) ); ?>
		</div><!-- .loop-description -->

	<?php } elseif ( is_archive() ) { ?>

		<h1 class="loop-title"><?php _e( 'Archives', 'neutro' ); ?></h1>

		<div class="loop-description">
			<?php echo wpautop( __( 'You are browsing the site archives.', 'neutro' ) ); ?>
		</div><!-- .loop-description -->
